...
type Kitalulus struct {
	source *legacy.Kitalulus
}

func NewKitalulus(client *http.Client, baseURL string) *Kitalulus {
	return &Kitalulus{
		source: legacy.NewKitalulus(client, baseURL),
	}
}

func (*Kitalulus) Source() string {
	return "kitalulus"
}

func (*Kitalulus) SupportsQueries() bool {
	return true
}
...
func (p *Kitalulus) Search(ctx context.Context, query domain.SearchQuery) ([]domain.RawJob, error) {
	jobs, err := p.source.Fetch(ctx, domain.Criteria{
		Positions: []string{query.Keyword},
		Locations: nonempty(query.Location),
	})
	return toRawJobs(jobs), err
}

type Dealls struct {
	source *legacy.Dealls
}

func NewDealls(client *http.Client, baseURL string) *Dealls {
	return &Dealls{
		source: legacy.NewDealls(client, baseURL),
	}
}

func (*Dealls) Source() string {
	return "dealls"
}

func (*Dealls) SupportsQueries() bool {
	return true
}
