...
func (h *FinanceHandler) Register(router fiber.Router) {
...
func (h *FinanceHandler) health(c *fiber.Ctx) error {
	if err := h.usecase.Ping(c.UserContext()); err != nil {
		return fiber.NewError(fiber.StatusServiceUnavailable, "database unavailable")
	}
	return c.SendStatus(fiber.StatusNoContent)
}

func (h *FinanceHandler) create(c *fiber.Ctx) error {
	...
	record, status, err := h.usecase.Create(c.UserContext(), input)
	if err != nil {
...
func (h *FinanceHandler) totals(c *fiber.Ctx) error {
	phone := strings.TrimSpace(c.Query("phone"))
	if phone == "" {
		return fiber.NewError(fiber.StatusBadRequest, "phone is required")
	}

	totals, err := h.usecase.Totals(c.UserContext(), phone)
	if err != nil {
...
func (h *FinanceHandler) recap(c *fiber.Ctx) error {
	phone := strings.TrimSpace(c.Query("phone"))
	if phone == "" {
		return fiber.NewError(fiber.StatusBadRequest, "phone is required")
	}

	records, err := h.usecase.Records(c.UserContext(), phone)
	if err != nil {
	...
	for _, item := range records {
		rows = append(rows, []any{
			item.CreatedAt.In(fixedWIB).Format("2006-01-02 15:04"),
			item.Type,
			item.Category,
			item.Description,
			item.Amount,
	...
	if err := file.SetColWidth(sheet, "A", "D", 22); err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, "could not create recap")
	}
...
var fixedWIB = time.FixedZone("WIB", 7*60*60)
